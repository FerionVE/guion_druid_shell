    ...
    for<'a> E::Backend: Backend<E,Renderer<'a>=Render<'a,E>>,
{
    fn fill_rect(&mut self, c: &mut E::Context<'_>) {
        ...
        let rect = bounds2rect(self.bounds);
        let color = self.style.color(&self.selector,c);
    ...
    fn fill_border_inner(&mut self, c: &mut E::Context<'_>) {
        ...
        let rect = bounds2rect(self.bounds);
        let color = self.style.color(&self.selector,c);
        let thickness = self.style.border(&self.selector,c).top;
    ...
    fn render_preprocessed_text(&mut self, text: &ETextLayout<E>, inner_offset: guion::util::bounds::Offset, c: &mut E::Context<'_>) {
        let r = unsafe{self.piet.get()};
        let rect = bounds2rect(self.bounds);
        let color = self.style.color(&self.selector,c);
